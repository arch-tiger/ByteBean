...
/**
 * 四参数方法调用字节码实现，为MethodInvoker生成高性能四参数方法调用字节码。
 * <p>
 * 该类专门处理四参数方法的调用，通过tableswitch指令实现方法索引到方法调用的快速分发。
 * 相比通用invoke方法，避免了参数数组的创建开销。
 * </p>
 * <p>
 * <b>API对应：</b> {@code Object invoke(int index, Object instance, Object arg1, Object arg2, Object arg3, Object arg4)}
 * </p>
 *
 * @author ZIJIDELU
 * @since 1.0.0
 */
public class MethodP4ByteCode implements Implementation {
     ...
     */
    public MethodP4ByteCode(Class<?> targetClass, List<MethodIdentify> methodIdentifyList) {
    ...
    public ByteCodeAppender appender(Target implementationTarget) {
        return (mv, implementationContext, instrumentedMethod) -> {
            ...
            // 注意：Slot 3, 4, 5, 6 被 arg1, arg2, arg3, arg4 占用，所以这里必须存入 Slot 7
            // ============================================================
            mv.visitVarInsn(Opcodes.ALOAD, 2); // Slot 2: Object instance
            mv.visitTypeInsn(Opcodes.CHECKCAST, owner);
            mv.visitVarInsn(Opcodes.ASTORE, 7); // Slot 7: TargetClass instance (Casted)
            ...
            for (int i = 0; i < methodIdentifyList.size(); i++) {
                ...
                // A: 【优化】直接从 Slot 7 加载已强转的 instance
